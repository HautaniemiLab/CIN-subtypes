...
chrlen <- read.table(file.path("data", "hg38.chrom.sizes.txt"),
                     sep="\t",stringsAsFactors = F)[1:24,]
centromeres <- read.table(file.path("data", "hg38.centromeres.txt"),
                     sep="\t", header=T)
...
bp5MB_discretize <- function(df, seed, plot = F)
{
  ...
  comp_to_group <- metrics_df %>% dplyr::select(cluster, group)
