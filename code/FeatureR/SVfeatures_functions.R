...
cosim <- function(samples_data)
{
  ...
  cosim_df <- cosim_df |>
    setNames(c("sample", "simple", "complex")) |>
    mutate(simple = as.integer(simple), 
           complex = as.integer(complex))
...
trater <- function(samples_data)
{
  ...
  trater_df <- trater_df |>
    setNames(c("sample", "interchr", "intrachr")) |>
    mutate(across(-sample, as.integer))
...
SV_categories <- function(samples_data)
{
  ...
  SV_classes <- SV_classes |>
    setNames(c("sample", "ins", "recip_inv", "recip_trans", "unbal_trans", "LINE")) |>
    mutate(across(-sample, as.integer))
